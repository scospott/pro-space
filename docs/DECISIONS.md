...
- En mode démo scripté seulement, un brouillon neuf est une maison : les sept pièces de la fixture existent dès l'arrivée sur l'étape 2, même si l'étape 1 n'a pas encore été remplie.
- Changement de type de logement : ajoute à la fin les pièces par défaut manquantes du nouveau type et retire seulement les pièces par défaut de l'ancien type restées vides (ni photo ni inventaire) et absentes de la nouvelle liste ; une pièce avec des photos, ajoutée ou renommée à la main n'est jamais retirée : les photos peuvent être prises avant de remplir l'étape 1.
...
- Proposition « Reprendre le brouillon / Repartir de zéro » (§3 règle 8) seulement au clic sur « Nouveau devis » du rail quand un brouillon non vide existe : le dialogue est porté par le rail, pas par une URL, il ne peut donc revenir ni au chargement ni entre les étapes. Ailleurs (page d'accueil, Mes devis), le brouillon reprend en silence ; `?nouveau=1` sert seulement à remplacer un devis envoyé par un brouillon neuf. Échap ferme le dialogue sans rien changer.
...
- Navigation libre entre les étapes 1 et 2 (stepper, « Continuer vers les photos », « Retour »), quel que soit l'état du formulaire, en écart au §3 règle 3 : Ethan peut photographier avant de remplir le client. Le dossier n'est validé qu'au clic sur « Générer le devis » ; en cas d'échec, retour à l'étape 1 avec les champs en erreur et « Il manque des informations client : … » dans la barre basse. L'indicateur `ui.erreursClientVisibles` est dans le store (persisté) pour survivre au changement d'écran et au rechargement.
